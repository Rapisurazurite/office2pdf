...
def ppt_to_pdf(powerPoint, inputFileName, outputFileName: str, formatType=17):
    outputFileName = os.path.splitext(outputFileName)[0]
    if outputFileName[-3:] != 'pdf':
        outputFileName += ".pdf"
    try:
        deck = powerPoint.Documents.Open(inputFileName)
...
def convert_files_in_folder(powerPoint, folder):
    ...
    pptFiles = [f for f in files if f.endswith((".doc", ".docx"))]  # 使用循环批量转换
